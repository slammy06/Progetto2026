...
void Sim::initBodies(const project::System &sys) {
  ...
  for (long unsigned int i = 0; i < sys.get_bodies().size(); ++i) {
    ...
    this->bodies[i].setOrigin({rad, rad});
    const float x =
        static_cast<float>(centerX + sys.get_bodies()[i].pos.x * scale);
    const float y =
        static_cast<float>(centerY + sys.get_bodies()[i].pos.y * scale);
...
void Sim::display_chart(const std::vector<point<double>> &LinMom,
  ...
  const std::vector<double> *values[] = {&AngMom, &linearX, &TotalEnergy};
  const sf::Color colors[] = {sf::Color::Blue, sf::Color::Red,
                              sf::Color::Yellow};
  const char *titles[] = {"Angular momentum Lz", "Linear momentum",
                          "Total energy"};
  ...
  for (int panel = 0; panel < 3; ++panel) {
    ...
    for (std::size_t i = 0; i < samples; ++i) {
      ...
      const float y =
          static_cast<float>(baseline - (*values[panel])[i] * scale);
      this->dot.setPosition({x, y});
      this->ChartWindow->draw(dot);
    }
    sf::Text maxLabel(std::to_string(maximumAbsolute(*values[panel])),
                      this->chartFont, 12);

    maxLabel.setFillColor(sf::Color::Black);
    maxLabel.setPosition({5.0f, baseline - panelHeight * 0.45f});

    this->ChartWindow->draw(maxLabel);
    sf::Text minLabel(std::to_string(minimumAbsolute(*values[panel])),
                      this->chartFont, 12);

    minLabel.setFillColor(sf::Color::Black);

    minLabel.setPosition({5.0f, baseline + panelHeight * 0.45f - 15.0f});
